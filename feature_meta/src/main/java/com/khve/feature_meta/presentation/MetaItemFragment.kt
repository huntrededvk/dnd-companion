...
class MetaItemFragment : Fragment() {
                ...
                viewModel.metaItemState.collect {
                    when(it) {
                        ...
                        is MetaItemState.MetaItem -> {
                            ...
                            setupViews(metaItem)
                            setupKarmaButtonsListeners(metaItem)
    ...
    private fun setupKarmaButtonsListeners(metaItem: MetaItem) {
        with(binding) {
            btnLike.setOnClickListener { viewModel.like(metaItem) }
            btnDislike.setOnClickListener { viewModel.dislike(metaItem) }
        }
    }
    ...
    private fun setupViews(metaItem: MetaItem) {
        metaItem.let { item ->
            with(binding) {
                ...
                // Set karma values
                val karma = item.likes.size - item.dislikes.size
